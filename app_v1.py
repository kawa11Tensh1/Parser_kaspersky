...
def get_vendor_id(connect, vendor_name):
    ...
    cursor.execute('''
    SELECT id FROM Products WHERE vendor_name = ?
    ''', (vendor_name))
...
def get_vulnerability_id(connect, kaspesky_id):
    ...
    cursor = connect.cursor()
    cursor.execute('''
    SELECT id FROM Vulnerabilities WHERE kaspesky_id = ?
    ''', (kaspesky_id))
...
def insert_vulnerability(connect, vulnerability_name, product_name, kaspersky_id):
    ...
    """
    product_id = get_product_id(connect, product_name, kaspersky_id)
    cursor = connect.cursor()
