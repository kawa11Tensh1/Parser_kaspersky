...
def creating_db():
    ...
    """
    # Устанавливаем соединение с базой данных (или создаём новую, если не существует)
    connect = sqlite3.connect("Kaspersky.sqlite")
    cursor = connect.cursor()

    # Создание таблицы Impacts
    cursor.execute('''
...
def get_product_id(connect, product_name):
    """
    Получает product_id из таблицы Products по имени продукта.
    """
    cursor = connect.cursor()
    cursor.execute('''
        SELECT product_id 
...
def get_vendor_id(connect, vendor_name):
    """
    Получает vendor_id из таблицы Vendors по имени вендора.
    """
    cursor = connect.cursor()
    cursor.execute('''
        SELECT vendor_id 
...
def get_vulnerability_id(connect, kaspersky_id):
    """
    Получает vulnerability_id из таблицы Vulnerabilities по идентификатору Kaspersky.
    """
    cursor = connect.cursor()
    cursor.execute('''
...
def insert_impact(connect, impact_name, kaspersky_id):
    """
    Вставляет данные в таблицу Impacts, проверяя, существует ли уже запись с таким же impact_name и vulnerability_id.
    """
    vulnerability_id = get_vulnerability_id(connect, kaspersky_id)
    cursor = connect.cursor()
...
def insert_vulnerability(connect, vulnerability_name, product_name, kaspersky_id):
    """
    Вставляет данные в таблицу Vulnerabilities, проверяя, существует ли уже такая уязвимость.
    """
    cursor = connect.cursor()
    ...
    if cursor.fetchone() is None:
        product_id = get_product_id(connect, product_name) or 0  # Устанавливаем product_id = 0, если не найден
...
def insert_product(connect, product_name, vendor_name):
    """
    Вставляет данные в таблицу Products, проверяя, существует ли уже такой продукт.
    """
    cursor = connect.cursor()
    ...
    if cursor.fetchone() is None:
        vendor_id = get_vendor_id(connect, vendor_name) or 0  # Устанавливаем vendor_id = 0, если не найден
...
def parse_products_pages(connect, base_url):
    ...
    while True:
        ...
        # Извлекаем и вставляем названия продуктов и вендоров в таблицу
        for container in containers:
...
def parse_vulnerabilities_pages(connect, base_url):
    ...
    while True:
        ...
        for container in containers:
            ...
            # Устанавливаем значение по умолчанию, если product_name не найден
            if not product_name:
...
def find_vulnerabilities(connect, product_name):
    """
...
def main():
    ...
    find_vulnerabilities(connect, "Microsoft Windows")
